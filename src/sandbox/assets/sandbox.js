...
if (componentPage) {
	...
	function showPreview(id) {
		...
		document
			.querySelectorAll("[data-variant-select]")
			.forEach((select) => select.classList.remove("current"));
		...
		document
			.querySelector(`[data-variant-select="${preview.dataset.variant}"]`)
	...
	copyCodeButton.addEventListener("click", (event) => {
		...
		const copyTarget = componentPage.querySelector(
			`[data-html="${codeBlockId}"] code`,
		);
	...
	if (eventInspector) {
		const componentEvents = eventInspector.dataset.eventInspector
			.split(",")
			.slice(0, -1);
